...
- Software domain: SM, PC, EG, TA, SR, SA, DR, IR, ST, EH, IM, ML (12)
- Data domain: SM, PC, EG, TA, SR, SA, DR (7)
...
Per the handbook:
- **Evidence-based:** Every "Yes" requires documented evidence
- **Cascading:** L2 requires complete L1; L3 requires complete L2
- **Partial = No:** Incomplete implementations don't count
- **Lookback period:** 3 months (practice must be current)

## State Management

### Before Assessment
1. Check `.verifhai/progress.json` for existing assessments
2. Show comparison to last full assessment if available
3. Load any existing evidence artifacts

### During Assessment
Track responses and evidence for each question:
```json
{
  "practices": {
    "SM": {
      "l1Questions": { "SM-SW-L1-Q1": true, "SM-SW-L1-Q2": false },
      "l2Questions": { ... },
      "evidence": [
        { "question": "SM-SW-L1-Q1", "path": "docs/security/strategy.md" }
      ]
    }
  }
}
...
### After Assessment
1. Save complete results to `.verifhai/progress.json`
2. Generate improvement roadmap
3. Record session with summary
