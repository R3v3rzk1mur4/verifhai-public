...
### Primary Data Source
Read progress from `.verifhai/progress.json` using schema at:
```
~/.claude/skills/Verifhai/state/progress.schema.json
```

### If No State File
If `.verifhai/progress.json` doesn't exist:
```
## HAI Security Status

No progress tracked yet for this project.

**Get Started:**
- `/verifhai start` - Begin your security journey (creates progress.json)
- `/verifhai assess` - Run quick assessment to establish baseline

**Or work on a specific practice:**
- `/verifhai practice sr` - Security Requirements
- `/verifhai practice ta` - Threat Assessment
```

### State File Location
`.verifhai/progress.json` in the current project root.
